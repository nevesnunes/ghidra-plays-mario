...
int main(int argc, char **argv) {
  f_inputs = fopen("/tmp/smb.inputs", "wb");
  f_dump = fopen("/tmp/smb.mem", "wb");
  ...
  for (;;) {
    ...
    fprintf(stderr, "CLT %02x%02x A:%02x X:%02x Y:%02x S:%02x P:%02x cy=%d\n", PCH, PCL, A, X, Y, S, P, cycles+2);
